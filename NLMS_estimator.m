...
% initial step-size
mu0 = 0.10;
epsilon = 10^-8; % small regularisation term used to avoid division by zero 
        ...
        for n=offset:length(ref.x)
            ...
            for jj=1:length(neuronSel) % using selected neurons
                for kk=[-1,0] % for std (kk=-1) and mean(kk=0) in 196
                    ...
                    mu = mu0 / (norm(outputTrain(elemOf,2*neuronSel(jj)+kk,n))^2 + epsilon);
...
figure
 subplot(2,1,1)
     plot(1:N,model(elemOf).ref.x,'linewidth',1.5,'DisplayName','reference'); hold on;
     plot(1:N,model(elemOf).pred.x,'linewidth',1.5,'DisplayName','prediction'); 
     title(sprintf('X Pos, Trial %d, Angle %d',whatTrial, whatAngle))
     ylabel('angle (degrees)')
     legend('show','Location','northwest')
 subplot(2,1,2)
     plot(1:N,model(elemOf).ref.y,'linewidth',1.5,'DisplayName','reference'); hold on;
     plot(1:N,model(elemOf).pred.y,'linewidth',1.5,'DisplayName','prediction');
