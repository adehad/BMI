...
%     if length(trial(1,1).spikes)>=300 
%         regionOfInterest(2) = length(trial(1,1).spikes) - modelParameters.maxLen; 
%     end
            ...
            for jj=1:gdNeur % using selected neurons
                predX(n) = predX(n) + W.x{incAngle}(jj,n)*tuningCurve{neuronSel(jj)}.meanVecMean(1,n) ...
                    *cosd(neuronPref(jj))*(activitySeg) ...
                    ;
                predY(n) = predY(n) + W.y{incAngle}(jj,n)*tuningCurve{neuronSel(jj)}.meanVecMean(1,n) ...
